...
export default function Bar(){
  ...
  {search &&
  <div className='h-screen w-screen fixed bg-black/20 top-0 left-0 z-[3]' onClick={()=> setSearch(false)}/>
    }
  <div className='relative flex flex-col items-center w-screen'>
    ...
    <input placeholder='🔍 Navigate, search, and run commands' className='ml-2 topicclui bg-transparent text-white min-w-[290px]' value={sections.length > 0 ? sections[sections.length - 1] : sections[0]} onChange={(e)=> {
    ...
    {selection.length > 0 ? 
    <>
    <p className='w-full text-lg font-semibold text-white/70 mb-3'>Results</p>
    {selection.map((cmd)=>{
      return <>
        <div key={cmd} onClick={()=> {
          cmd.data.run()
    ...
    <div className='text-white h-full w-full flex flex-col items-center'>
          <p className='w-full text-lg font-semibold text-white/70 mb-3'>Query not found. Here are some suggestions:</p>
      {
